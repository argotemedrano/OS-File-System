...
void test_os(int argc, char** argv){
    ...
    if(argc>2 && atoi(argv[2]) == 3){
        OS t;
        t.create_item("z",false);t.create_item("b",true);
    	t.create_item("c",true);t.create_item("a",false);
    	t.create_item("d",false);t.create_item("f",false);
    	t.ls("-d");cout<<endl;
    	t.cd_dir("o");//should say error
    	t.cd_dir("c");
    	t.create_item("f",false);t.create_item("k",true);
    	t.create_item("p",true);t.create_item("d",false);
    	t.ls("-d");cout<<endl;
    	t.ls("-sort=size");cout<<endl;
    	t.file("c",false);//should say error
    	t.file("d",false);//here you type in "this is a test write to d"
    	t.file("k",true);//should say error
    	t.file("d",true);//should say "this is a test write to d" size 25
    	t.file("f",false);//here you type in "writes to f"
    	t.file("f",true);//should say "writes to f" size 11
    	t.ls("-d");cout<<endl;
    	t.ls("-sort=size");cout<<endl;
    	t.cd_dir("..");
    	t.ls("-r");cout<<endl;
